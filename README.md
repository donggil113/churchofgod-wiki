...
1. [Pages CMS](https://app.pagescms.org/)에서 GitHub로 로그인하고 이 저장소만 선택하여 앱을 설치합니다.
2. **문서** 목록에서 수정할 글을 열어 제목, 요약, 분류, 본문을 편집하고 저장합니다.
3. GitHub 저장소의 `main` 브랜치에 변경이 저장되면 게시 작업이 자동 실행됩니다.

CMS를 연결하기 전에도 GitHub 웹 화면에서 `content/pages/*.json` 파일을 직접 수정할 수 있습니다. 대문 역시 이 폴더의 문서 중 하나입니다. 사진을 새로 올릴 때는 `media/` 폴더를 사용하세요. 원본 위키에서 가져온 사진은 원본 서버 주소를 사용하므로, 독립 보관이 필요하면 별도 자산 이전 작업이 필요합니다.
